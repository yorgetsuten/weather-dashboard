...
export function Client({ passedLocation }: { passedLocation: string | null }) {
  const [location, setLocation] = useState<string>(
    passedLocation ?? 'London, United Kingdom'
  )
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      const location = getStringifiedLocation(
        (await request('current', `${latitude},${longitude}`)).location
      )
  ...
  return (
    <div className='relative flex items-center gap-[6px]'>
      ...
      <Tooltip>Is this your current location?</Tooltip>
    </div>
  )
}
